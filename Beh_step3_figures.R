...
df.hold <- read.table("/Volumes/Yorick/STT_reml/Analyses/roiAnalysis/sub_stats/Betas_Table_T1.txt",header=T, sep = "\t")
df.hold <- as.matrix(df.hold)

df.reorganize <- matrix(NA,nrow=dim(df.hold)[1],ncol=dim(df.hold)[2])
df.reorganize[,1:3] <- df.hold[,4:6] #LMulti
df.reorganize[,4:6] <- df.hold[,1:3] #LCA1
...
# save out as tiff
tiff(paste0(outDir,"Fig_Plot_Sub_T1.tiff"), height = 5.5, width = 7, units = 'in', res=300)

# draw graph - ylim max was hardcoded for the legend issue
par(mar=c(5,5,4,2),family="Times New Roman")
hold.graph <- barplot(matrix(df.plotable[1,],nrow=3), beside=T, ylim=c(graph_min,0.25),ylab="Beta Estimate",cex.lab=1.5,legend=c("Hit","FA","CR"),col=h.colors)
abline(h=0)
...
mtext(c("CA2-CA3-DG", "CA1"), side=1, at=hold.graph[2,], cex=1.2)
title(main=list("Left HC Subfields: Test1", cex=2))

# manually build matrix of sig levels, for determining how many stars to use
# row 1 = Hit-FA; row 2 = FA-CR; row 3 = Hit-CR
# columns for each cluster
# 1 = *; 2 = **; 3 = ***
sig_matrix <- matrix(0,nrow=dim(hold.graph)[1],ncol=dim(hold.graph)[2])

sig_matrix[1,1] <- 1 # LMulti Hit-FA *
sig_matrix[3,1] <- 3 # LMulti Hit-CR ***
sig_matrix[3,2] <- 2 # LCA1 H-C **
  ...
  c<-1; for(a in perm_hold){
    ...
    if(sig_matrix[c,count] != 0){
      ...
      if(pos_value == T){
        ...
        segment_y <- hold_start*0.2*graph_max
...
df.reorganize <- matrix(NA,nrow=dim(df.hold)[1],ncol=dim(df.hold)[2])
df.reorganize[,1:3] <- df.hold[,10:12] #lTPJ
df.reorganize[,4:6] <- df.hold[,7:9] #lvmPFC
df.reorganize[,7:9] <- df.hold[,13:15] #ldmPFC
df.reorganize[,10:12] <- df.hold[,4:6] #lMFG
df.reorganize[,13:15] <- df.hold[,1:3] #lAG
...
segments(hold.graph, df.plotable[1,]-df.plotable[2,], hold.graph, df.plotable[1,]+df.plotable[2,])
arrows(hold.graph, df.plotable[1,]-df.plotable[2,], hold.graph, df.plotable[1,]+df.plotable[2,], lwd = 1, angle = 90, code = 3, length = 0.05)
mtext(c("LTPJ","LVMPFC","LDMPFC","LMFG","LAG"), side=1, at=hold.graph[1,]+0.5, cex=1.2)
...
sig_matrix[1,1] <- 3 # LTPJ Hit-FA ***
sig_matrix[3,1] <- 2 # LTPJ Hit-CR **
sig_matrix[1,2] <- 1 # LVMPFC Hit-FA *
sig_matrix[3,2] <- 2 # LVMPFC Hit-CR **
sig_matrix[2,3] <- 3
sig_matrix[3,3] <- 3
sig_matrix[2,4] <- 2
sig_matrix[3,4] <- 3
